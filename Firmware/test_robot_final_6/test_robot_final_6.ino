...
volatile float vRobot = 0;
volatile float err_center = 0;
...
void OnDataRecv(const esp_now_recv_info_t * info, const uint8_t *incomingDataPtr, int len) {
  if (len == sizeof(incomingData)) {
    ...
    vRobot = incomingData.v_robot;
    err_center = incomingData.errorLine;
...
void handleRoot() {
  ...
  html += ".st-done::before { content: 'X'; margin-right: 10px; font-weight: bold; }"; // Thêm dấu tick
  ...
  html += ".st-pending::before { content: 'O'; margin-right: 10px; font-size: 20px; }"; // Dấu chấm tròn
  ...
  // CSS cho Error Box
  html += "#errbox { display: block; font-size: 1.5em; font-weight: bold; text-align: center; color: white; padding: 5px 0; border-radius: 4px; transition: background 0.3s; }";
  ...
  // --- VELOCITY & ERROR LINE (THÊM MỚI) ---
  html += "<div class='dash-grid'>";
  html += "<div class='card'><div class='card-title'>Velocity (cm/s)</div><div class='dash-val' id='vel'>0.0</div></div>";
  html += "<div class='card'><div class='card-title'>Abs Error Line</div><div id='errbox' style='background:#ccc;'>0.0</div></div>";
  html += "</div>";
  ...
  // --- HIỂN THỊ Vận tốc và Lỗi Line (THÊM MỚI) ---
  // Velocity
  html += "document.getElementById('vel').innerText=d.velocity.toFixed(1);";

  // Error Line (Abs value + Color logic)
  html += "let eb = document.getElementById('errbox');";
  html += "let absErr = Math.abs(d.err_center);";
  html += "eb.innerText = absErr.toFixed(1);";
  html += "if(absErr < 12) eb.style.background = '#28a745';"; // Xanh: < 12
  html += "else if(absErr <= 15) eb.style.background = '#ffc107';"; // Vàng: 12 <= absErr <= 15
  html += "else eb.style.background = '#dc3545';"; // Đỏ: > 15

/*
 * ESP32 ROBOT - SLAVE (FIXED COMMAND LOGIC)
 * ==================================================
 * - MAC Controller (Master): 6C:C8:40:34:7A:68
 * - Fix: Xử lý lệnh bằng biến cờ (flag) thay vì String để tránh lỗi.
 */
...
// ==========================================================
uint8_t masterAddress[] = {0x6C, 0xC8, 0x40, 0x34, 0x7A, 0x68};
...
// Biến cờ xử lý lệnh (Volatile để dùng trong ngắt)
volatile bool newCommandReceived = false;
volatile char commandChar = ' ';

// ==========================================================
// 2. HARDWARE (GIỮ NGUYÊN)
// ==========================================================
...
// ===================== CALLBACK ESP-NOW (SẠCH SẼ HƠN) =====================
void OnDataRecv(const esp_now_recv_info_t * info, const uint8_t *incomingData, int len) {
  memcpy(&incomingCmd, incomingData, sizeof(incomingCmd));
  // Chỉ lưu lệnh vào biến tạm, không xử lý logic nặng ở đây
...
void sendTelemetry() {
  ...
  // --- THÊM ĐOẠN NÀY ĐỂ GỬI GIÁ TRỊ CẢM BIẾN ---
  // norm_sensor là mảng chứa giá trị đã chuẩn hóa (0-1000)
  myData.s1 = norm_sensor[0];
  myData.s2 = norm_sensor[1];
  myData.s3 = norm_sensor[2];
  myData.s4 = norm_sensor[3];
  myData.s5 = norm_sensor[4];
...
void setup(){
  ...
  memset(&peerInfo, 0, sizeof(peerInfo)); // Xóa rác bộ nhớ
...
// ===================== LOOP =====================
void loop(){
  // ================= 1. XỬ LÝ LỆNH TỪ ESP-NOW/SERIAL =================
  if (newCommandReceived) {
    newCommandReceived = false; // Reset cờ
    if (commandChar == 'g') {
      currentState = MOVING; 
      currentShipStatus = MOVING_TO_LOADING; 
      cargoColor = 0; isIntersection = false;
      resetPID();
      Serial.println(">>> CMD: GO (Received from WiFi)");
    }
    else if (commandChar == 's') {
      currentState = STOP; 
      currentShipStatus = STOPPED_BY_USER;
      Serial.println(">>> CMD: STOP (Received from WiFi)");
    }
  }

  // Debug qua Serial (Dự phòng)
  if (Serial.available()) {
    char c = Serial.read();
    if(c == 'g') { commandChar = 'g'; newCommandReceived = true; }
    if(c == 's') { commandChar = 's'; newCommandReceived = true; }
  }
  ...
  if (currentState == MOVING) {
    if (isFinish) {
      ...
      currentState = FINISH_SHIPPING; currentShipStatus = FINISHED_DELIVERY;
      return;
    }
    if (cargoColor == 0 && isPickupZone) {
      setMotors(-20, -20); resetPID();
      currentState = WAIT_LOADING; currentShipStatus = WAIT_AT_LOADING;
      stateStart = now; Serial.println("Arrived Pickup Zone");
      return;
    }
  ...
  switch(currentState){
    ...
    case WAIT_LOADING: {
      ...
      if (colorStart > 0 && (now - colorStart >= 3000)) {
        ...
        lastColor = 0; colorStart = 0;
        Serial.println(cargoColor == 1 ? "Got RED Cargo" : "Got BLUE Cargo");
        delay(1000); 
    ...
    case INTERSECTION_DECISION: 
      if (cargoColor == 1) { // RED -> LEFT
        resetPID();
        setMotors(-150, 150); 
        if (now - stateStart >= 350) { resetPID(); currentState = MOVING; currentShipStatus = MOVING_TO_ENDPOINT; }
      } 
    ...
    case FINISH_SHIPPING: 
      setMotors(-20, -20); currentState = STOP; 
      break;
  } 
}